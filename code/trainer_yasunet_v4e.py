...
def lambda_spectral_for_epoch(epoch):
    if epoch <= 15:
        ...
        return 0.07   # v4e: half the lambda=0.10 tax, well below basin threshold
...
def train(cache_dir, stats_path, output_dir, tier, seed):
    ...
    run_name = f"yasunet_v4e_tier{tier}_seed{seed}"
    run_dir  = Path(output_dir) / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    config = {
        "model": "yasunet_v4e", "tier": tier, "seed": seed,
        ...
        "lambda_spectral_schedule": {"1-15": 0.05, "16-50": 0.07},
