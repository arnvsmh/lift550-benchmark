...
yasunet_v3_fskip.py
===================
YASU-Net v3 + FourierSkip rewrite — Physics-informed turbulence super-resolution.
...
class FourierSkip(nn.Module):
    """
    v3-fskip variant: proper frequency-domain skip connection.

    Applies a LEARNED COMPLEX 1x1 convolution directly on the Fourier
    coefficients (channel-mixing in frequency space), then transforms back
    to the spatial domain via a single irfft2. This actually processes phase
    and magnitude jointly in the frequency domain, which the v2 construction
    (separate irfft of real and of imag) did not do correctly.

    Complex 1x1 conv: for each (out,in) channel pair, a complex weight
    w = w_r + i*w_i multiplies the complex spectrum. Implemented via the
    standard complex matmul identity.
    """
    def __init__(self, channels):
        super().__init__()
        scale = 0.02
        self.w_r = nn.Parameter(scale * torch.randn(channels, channels))
        self.w_i = nn.Parameter(scale * torch.randn(channels, channels))
        # spatial residual mix after inverse transform
    ...
    def forward(self, x):
        ...
        Returns:
            fourier_skip: [B, C, H, W] frequency-processed features
        """
        B, C, H, W = x.shape
        x_f32 = x.float()
        x_ft = torch.fft.rfft2(x_f32)                # [B, C, H, W//2+1] complex
        xr, xi = x_ft.real, x_ft.imag
        wr, wi = self.w_r.float(), self.w_i.float()
        # complex channel mixing: (xr+i*xi) @ (wr+i*wi) over input channel dim
        out_r = torch.einsum("bixy,oi->boxy", xr, wr) - torch.einsum("bixy,oi->boxy", xi, wi)
        out_i = torch.einsum("bixy,oi->boxy", xr, wi) + torch.einsum("bixy,oi->boxy", xi, wr)
        out_ft = torch.complex(out_r, out_i)
        feat = torch.fft.irfft2(out_ft, s=(H, W))    # [B, C, H, W] real
        return self.proj(feat.to(x.dtype))
