...
def lambda_spectral_for_epoch(epoch):
    if epoch <= 15:
        ...
        return 0.10   # measured ramp: lo run held this for 20 epochs, no basin jump
...
def train(cache_dir, stats_path, output_dir, tier, seed):
    ...
    run_name = f"yasunet_v4d_tier{tier}_seed{seed}"
    run_dir  = Path(output_dir) / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    config = {
        "model": "yasunet_v4d", "tier": tier, "seed": seed,
        ...
        "lambda_spectral_schedule": {"1-15": 0.05, "16-50": 0.10},
