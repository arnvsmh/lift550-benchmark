...
def train(cache_dir, stats_path, output_dir, tier, seed):
    ...
    print(f"YASU-Net v4c  params: {n_params:,}")
    ...
    print(f"  Spectral loss: EVAL-ALIGNED radial (k_max={spec_loss_fn.k_max}), "
          f"ramp 0.05->0.15->0.30, validation fixed at lambda={lam_final}")
    print(f"  Charbonnier: PLAIN (near-wall weighting removed)")

    run_name = f"yasunet_v4clo_tier{tier}_seed{seed}"
    run_dir  = Path(output_dir) / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    config = {
        "model": "yasunet_v4clo", "tier": tier, "seed": seed,
        ...
        "lambda_spectral_schedule": {"1-15": 0.05, "16-35": 0.10, "36-50": 0.15},
